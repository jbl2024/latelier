...
export default {
  ...
  methods: {
    ...
    async deleteProject(project) {
      try {
        const res = await this.$confirm(this.$t("Delete project?"), {
          title: project.name,
    ...
    async leaveProject(project) {
      try {
        await this.$confirm(this.$t("Leave project?"), {
          title: project.name,
          cancelText: this.$t("Cancel"),
          confirmText: this.$t("Leave project")
        });
        await Meteor.callAsync("projects.leave", { projectId: project._id });
      } catch (error) {
        this.$notifyError(error);
      }
    },
    ...
    async removeFromFavorites(user, projectId) {
      ...
      try {
        await Meteor.callAsync("projects.removeFromUserFavorites", { projectId: projectId, userId: user._id });
      } catch (error) {
        this.$notifyError(error);
      }
    },
    ...
    async cloneProject(project) {
      try {
        const res = await this.$confirm(this.$t("Do you really want to clone this project?"), {
        ...
        if (res) {
          await Meteor.callAsync("projects.clone", { projectId: project._id });
