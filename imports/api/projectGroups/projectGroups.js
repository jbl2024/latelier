...
Meteor.methods({
  "projectGroups.create"(organizationId, name) {
    ...
    const projectGroupId = ProjectGroups.insert({
      organizationId,
      name,
      createdAt: new Date(),
  ...
  "projectGroups.updateName"(projectGroupId, name) {
    ...
    ProjectGroups.update({ _id: projectGroupId }, { $set: { name } });
  },
  ...
  "projectGroups.addProject"(projectGroupId, projectId) {
    ...
    if (
      ProjectGroups.find({ _id: projectGroupId, projects: projectId }).count()
      > 0
    ) {
      return;
    }
